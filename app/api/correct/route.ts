...
- **社内LINE（先輩・目上向け）**: 社内の先輩・上司へのLINEメッセージ。敬語は崩さないが、メールのような堅苦しさは不要。「お世話になっております」等の定型挨拶や署名は付けない。短く読みやすい文で、礼儀と親しみやすさのバランスを取る
- **社内LINE（同僚・部下向け）**: 社内の同僚・部下へのLINEメッセージ。フランクで簡潔。ですます調を基本としつつ距離感は近めに。定型挨拶や署名は付けない。部下宛でも命令口調にならず、感じの良い頼み方・伝え方にする
